...
Hand** Dealer::getPlayerHands()
{
    ...
    for (int i = 0; i < numPlayers; i++) {
        playerHands[i] = players[i]->getPlayerHand();  // Cambiado de getPlayersHand a getPlayerHand
    }
...
void Dealer::dealCards()
{
    for (int i = 0; i < numPlayers; i++) {
        for (int j = 0; j < NUM_CARDS_PER_PLAYER; j++) {
           ...
           players[i]->getPlayerHand()->playerHand[j] = Card(tempCard.getValue(), tempCard.getSymbol());
        }
    }
